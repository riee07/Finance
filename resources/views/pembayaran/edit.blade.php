...
@section('content')
    ...
    <form action="{{ route('pembayaran.update', $pembayarans->id_pembayaran) }}" method="POST">
        @csrf @method('PUT')
        ...
        <input type="date" name="tanggal_pembayaran" class="border p-2 w-full" value="{{ $pembayarans->tanggal_pembayaran }}" required>

        <label class="block">Jumlah Pembayaran:</label>
        <input type="number" name="jumlah_pembayaran" class="border p-2 w-full" value="{{ $pembayarans->jumlah_pembayaran }}" required>
        ...
        <a href="{{ route('pembayaran.index') }}" class="ml-2 text-gray-600">Batal</a>
    </form>
@endsection
